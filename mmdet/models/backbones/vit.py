...
class Attention(MultiheadAttention):
    ...
    def forward(self, query, H, W, key=None, value=None, **kwargs):
        if key is None:
            key = query
        if value is None:
            value = query
        return super().forward(query, key, value, need_weights=False, **kwargs)[0]
...
class Block(nn.Module):
    ...
    def forward(self, x, H, W, **kwargs):
        ...
        def _inner_forward(x):
            if self.layer_scale:
                x = x + self.drop_path(self.gamma1 * self.attn(self.norm1(x), H, W, **kwargs))
                x = x + self.drop_path(self.gamma2 * self.mlp(self.norm2(x)))
            else:
                x = x + self.drop_path(self.attn(self.norm1(x), H, W, **kwargs))
    ...
    def noattn(self, x, H, W, **kwargs):
        if self.layer_scale:
            x = x + self.drop_path(self.gamma1 * self.attn.noattn(self.norm1(x), H, W, **kwargs))
            x = x + self.drop_path(self.gamma2 * self.mlp(self.norm2(x)))
        else:
            x = x + self.drop_path(self.attn.noattn(self.norm1(x), H, W, **kwargs))
            x = x + self.drop_path(self.mlp(self.norm2(x)))
...
class TIMMVisionTransformer(BaseModule):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, residual_indices=[], embed_dim=768,
                 ...
                 act_layer=nn.GELU, window_attn=False, window_size=14, with_cp=False, pretrained=None, init_cfg=None):
        """
        Args:
    ...
    def init_weights(self):
        ...
        if self.init_cfg is None:
            # raise ValueError('Please specify `Pretrained` in `init_cfg` in '
