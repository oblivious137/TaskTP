...
class SelectiveViTAdapter(SelectiveVisionTransformer):
    ...
    def forward(self, x, need_loss=False):
        ...
        num_loss = 0
        full_ratios = []
        self.forward_dict = dict(masks=list())
        for i, layer in enumerate(self.interactions):
            indexes = self.interaction_indexes[i]
            if not self.inherit_mask:
                x, c, layer_ratio_loss, has_loss, ratios = layer(x, c, indexes,
                             deform_inputs1, deform_inputs2, H, W,
                             self.blocks, self.selective_modules, self.keep_ratio)
            else:
                x, c, layer_ratio_loss, has_loss, prev_decision, ratios = layer(x, c, indexes,
                             deform_inputs1, deform_inputs2, H, W,
            ...
            full_ratios.extend(ratios)
            self.forward_dict['masks'].extend(layer.forward_dict['masks'])
        ...
        f4 = self.norm4(c4)
        if need_loss:
            ret_dict = {'backbone.ratio_loss': ratio_loss / num_loss}
            for i, ratio in enumerate(full_ratios):
                ret_dict[f'backbone.ratio_{i}'] = ratio
            return [f1, f2, f3, f4], ret_dict
        else:
            return [f1, f2, f3, f4]
