...
class MSDeformAttn(nn.Module):
    def __init__(self, d_model=256, n_levels=4, n_heads=8, n_points=4, ratio=1.0):
        ...
        """
        assert abs(ratio-1) < 1e-6, 'not supported yet'
        super().__init__()
...
class Extractor(nn.Module):
    def __init__(self, dim, num_heads=6, n_points=4, n_levels=1, deform_ratio=1.0,
        ...
        super().__init__()
        assert deform_attn, 'only support deformable attention'
            ...
            if i < n_skip:
                ...
                if self.training:
                    ...
                    if x.shape[0] == 1:
                        ...
                        self.forward_dict['masks'].append(selector)
                    else:
                        selector, diff_selector = selective_modules[i - n_skip](x)
                ...
                ratios.append(selector.float().mean())
        c = self.extractor(query=c, reference_points=deform_inputs2[0],
                           feat=x, spatial_shapes=deform_inputs2[1],
                           level_start_index=deform_inputs2[2], H=H, W=W)
        if self.extra_extractors is not None:
            for extractor in self.extra_extractors:
                c = extractor(query=c, reference_points=deform_inputs2[0],
                              feat=x, spatial_shapes=deform_inputs2[1],
                              level_start_index=deform_inputs2[2], H=H, W=W)
        return x, c, layer_ratio_loss, has_loss, ratios
