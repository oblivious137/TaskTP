...
def parse_args():
    ...
    parser.add_argument(
        '--out',
        help='output benchmark metrics to a txt file')
    parser.add_argument(
        '--cfg-options',
        nargs='+',
        action=DictAction,
...
def main():
    args = parse_args()
    # extract config from log file
    if args.config.endswith('.log'):
        with open(args.config, 'r') as f:
            lines = f.readlines()
        start_line, end_line = -1, -1
        for i, line in enumerate(lines):
            if "mmengine - INFO - Config:" in line:
                start_line = i + 1
            elif start_line > 0 and "mmengine - INFO - " in line:
                end_line = i
                break
        temp_file = 'temp.py'
