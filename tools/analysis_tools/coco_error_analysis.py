...
def makebarplot(rs, ps, outDir, class_name, iou_type):
    ...
    width = 0.60  # the width of the bars
    rects_list = []
    figure_title = iou_type + '-' + class_name + '-' + 'precision bar plot'
    for i in range(len(types) - 1):
        type_ps = ps[i, ..., 0].copy()
        type_ps[type_ps <= 0] = 10
        precition = type_ps.min(axis=0)
        rects_list.append(
            ax.bar(
                x - width / 2 + (i + 1) * width / len(types),
                precition,
    ...
    width = 0.60  # the width of the bars
    rects_list = []
    figure_title = iou_type + '-' + class_name + '-' + 'recall bar plot'
    for i in range(len(types) - 1):
        type_ps = ps[i, ..., 0].copy()
        recall = list()
        for j in range(type_ps.shape[1]):
            try:
                recall.append(np.where(type_ps[:, j] > 0)[0][-1] * 0.01)
            except IndexError:
                recall.append(0)
